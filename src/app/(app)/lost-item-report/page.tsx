...
const lostItemSchema = z.object({
  ...
  location: z.string().min(1, 'Last known location is required'),
...
export default function LostItemReportPage() {
  ...
  const onSubmit = async (data: LostItemFormData) => {
    setIsLoading(true);
    try {
      const itemRef = doc(collection(db, 'lost-items'));
      ...
      await setDoc(doc(db, 'lost-items', itemId), docData);
      toast({
        title: 'Lost Item Posted',
        description: 'Your lost item has been posted publicly.',
      });
      router.push('/search-match');
    } catch (error: any) {
      toast({
        title: 'Error',
        description: error.message || 'Failed to post lost item.',
        variant: 'destructive',
      });
      console.error('Error posting lost item:', error.message);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      <h1 className="mb-6 text-center text-3xl font-bold tracking-tight text-foreground">Post Lost Item</h1>
      <Form {...form}>
